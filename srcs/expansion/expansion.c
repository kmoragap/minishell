...
/*   Updated: 2024/04/18 16:05:04 by kmoraga          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//        return strdup("");
    ...
    while (env[i] != NULL) {
        ...
        if (strcmp(var, token) == 0) {
            nested_value = expand_token(value, env);
...
void expand_cmd(t_token **token, char **env) 
{
    ...
    if (check_expand_var((*token)->cmd)) 
    {
        ...
        expanded_cmd = expand_token((*token)->cmd, env);
...
void expand_args(t_token **token, char **env) 
{
    ...
    while((*token)->args[i] != NULL)
    {
        ...
        {
            //funcion para alloc mem in expanded variable
            expanded_arg = expand_token((*token)->args[i], env);
